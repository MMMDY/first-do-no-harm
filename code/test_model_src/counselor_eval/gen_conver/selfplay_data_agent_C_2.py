...
class MakeData:
    def __init__(self, model_type="api", model_name=None, api_base=None, api_key=None, 
                 ...
                 group="B", keep_c_internal_history=False, rewrite_start_k=1):
        ...
        self.rewrite_start_k = rewrite_start_k
        if self.group not in ["B", "C"]:
            raise ValueError("group 仅支持 'B' 或 'C'")
        if self.rewrite_start_k < 1:
            raise ValueError("rewrite_start_k 必须 >= 1")

        self.init_message = "Hello. How are you feeling today?"

        self.paths = {
            'output_path': output_data_path or '/WX24061/lzy/test_model/counselor_eval/gen_conver/output_data/test/test_5.jsonl',
            ...
            "supervisor_prompt_path": '/WX24061/lzy/test_model/counselor_eval/gen_conver/prompt/supervisor_promp_rewrite.txt',
    ...
    def run_conversation_c_group(self, patient_prompt, counselor_prompt, supervisor_prompt, test_mode=False):
        ...
        if self.rewrite_start_k >= max_turns:
            raise ValueError(f"rewrite_start_k 必须小于 max_turns，当前为 k={self.rewrite_start_k}, max_turns={max_turns}")
        ...
        for turn_idx in range(max_turns):
            therapist_utterance_idx = turn_idx + 1
            ...
            if therapist_utterance_idx >= self.rewrite_start_k:
                ...
                counselor_revised = counselor_draft
            ...
            turn_trace.append({
                "therapist_utterance_idx": therapist_utterance_idx,
                "rewritten": therapist_utterance_idx >= self.rewrite_start_k,
...
def parse_arguments():
    ...
    parser.add_argument('--rewrite_start_k', type=int, default=1,
                        help='仅在 C 组生效：从咨询师第k句开始（含第k句）进行改写，需满足 k < max_turns')
...
def main():
    ...
    try:
        data_maker = MakeData(
            ...
            rewrite_start_k=args.rewrite_start_k
        )
