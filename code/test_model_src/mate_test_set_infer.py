...
# MATE测试集infer，使用多线程并发调用API生成预测结果，并保存到文件中。

SYSTEM_PROMPT="You are a psychological supervisor. You first thinks about the reasoning process in the mind and then provides the user with the answer."
...
class Critic_Type_Classify:
    ...
    def generate_predictions(self, input_file: str, output_file: str, max_workers: int, num_samples: int = None, enable_thinking: bool = False, temperature: float = 0.01):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_item = {
                executor.submit(self.get_prediction, SYSTEM_PROMPT, item.get("history", ""), enable_thinking, temperature): item
                for item in data
            }
            ...
            for future in tqdm(as_completed(future_to_item), total=len(data), desc="生成预测中"):
                ...
                result_item["instruction"] = SYSTEM_PROMPT
...
def run(max_workers: int = 5, num_samples: int = 3):
    """
    执行完整的预测和评估流程。
    """

    prompt_template_path = "/WX24061/lzy/grpo/grpo_v3_templatte.txt" # /WX24061/models/qwen3-14b
    ...
    predictions_output_dir = "/WX24061/lzy/grpo/"
    #predictions_output_dir= "/WX24061/lzy/test_model/mistake_classify_mate5_v1/"
    predictions_output_file = predictions_output_dir + "Qwen3-8B-Instruct-mate5-250-grpo_plugin3-370-plugin4-630-test.json"
